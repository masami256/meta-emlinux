...
from lib.python.package_info import PackageMap
...
class NvdCveNvdInfo:
    def __init__(self, cveid: str, src_pkg_name: str, bin_pkg_names: list[str], version: str, summary: str, scorev2: str, scorev3: str, vector: str, vector_string: str, status: str) -> None:
        self.cveid = cveid
...
class NvdCveProductInfo:
    def __init__(self, cveid: str, src_pkg_name: str, vendor: str, product: str, version_start: str, operator_start: str, version_end: str, operator_end: str) -> None:
        self.cveid = cveid
        self.src_pkg_name = src_pkg_name
        self.vendor = vendor
...
class CveCheckMergedList():
    ...
    def add_data(self, src_pkg_name: str, cveid: str, cve_data: dict, priority: int) -> None:
            ...
            if src_pkg_name in self.merged:
                for cveid in ignore_list[src_pkg_name]:
                    if cveid in self.merged[src_pkg_name]:
                        self.merged[src_pkg_name][cveid]["cve_info"].status = CveStatus.CVE_STATUS_PATCHED


class NvdCveInfoListCreator():
    def __init__(self, cve_data_dir: str, package_map: PackageMap, kev_info_list: KevInfoList) -> None:
    ...
    def create_cve_info_list(self, cve_check_merged_list: CveCheckMergedList) -> NvdCveNvdInfoList:
        ...
        for src_pkg_name in cve_check_merged_list:
            for cveid in cve_check_merged_list[src_pkg_name]:
                ...
                status = cve_check_merged_list[src_pkg_name][cveid]["cve_info"].status
                ni = self._get_cve_information(cveid, src_pkg_name, self.package_map[src_pkg_name]["bin_pkg_names"], self.package_map[src_pkg_name]["version"], status)
                if cveid in self.kev_info_list:
                    ni.set_kev(self.kev_info_list.get_known_ransomware_campaign_use(cveid))

                self.nvd_info_list.add_nvd_cve_nvd_info(ni)
        self.conn.close()

    def _get_cve_information(self, cveid: str, src_pkg_name: str, bin_pkg_names: list[str], version: str, status: str) -> NvdCveNvdInfo:
        c = self.conn.cursor()
        sql = sql = f"SELECT VULNSTATUS, SUMMARY, SCOREV2, SCOREV3, VECTOR, VECTORSTRING FROM NVD WHERE ID=\"{cveid}\""
        ...
        return NvdCveNvdInfo(cveid, src_pkg_name, bin_pkg_names, version, summary, scorev2, scorev3, vector, vector_string, status)
