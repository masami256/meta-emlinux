...
from lib.python.package_info import PackageInfo, PackageInfoList, PackageMap
...
class CveReporter:
    def __init__(self, output_base_dir: str, image_name: str, package_map: PackageMap) -> None:
        self.output_base_dir = output_base_dir
        self.image_name = image_name
        self.package_map = package_map

    def write_report(self, formats: str, cve_info_list: NvdCveNvdInfoList, installed_packages: PackageInfoList) -> None:
        ...
        for src_pkg_name in cve_info_list:
            ...
            with open(filename, "w") as f:
                for ci in cve_list:
                    ...
                    f.write(f"VECTOR STRING: {ci.vector_string}\n")
    ...
    def _write_json_report(self, cve_info_list: NvdCveNvdInfoList, installed_packages: PackageInfoList) -> list[str]:
        ...
        for src_pkg_name in cve_info_list:
            cve_list = cve_info_list[src_pkg_name]
            has_cve = src_pkg_name in installed_packages
            data = self._create_json_data_for_package(src_pkg_name, cve_list, installed_packages[src_pkg_name], has_cve)
    ...
    def _create_json_data_for_package(self, src_pkg_name: str, cve_info: NvdCveNvdInfo, package_info: PackageInfo, has_cve: bool):
        cvesInRecord = "Yes" if has_cve else "No"
        ...
        version = self.package_map[src_pkg_name]["version"]
        bin_pkg_names = self.package_map[src_pkg_name]["bin_pkg_names"]
    ...
    def _create_issue_data(self, cve_info: NvdCveNvdInfo) -> list[dict]:
        ...
        for ci in cve_info:
            ...
            data["VECTOR STRING"] = ci.vector_string
