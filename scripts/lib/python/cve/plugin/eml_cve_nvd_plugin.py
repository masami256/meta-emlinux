...
class EmlNVDPlugin(EmlCvePlugin):
    def __init__(self, cve_data_dir: str, args: Any, bitbakeinfo: Any, installed_packages: PackageInfoList):
        super().__init__("EmlNVDPlugin", 1, cve_data_dir, args, bitbakeinfo, installed_packages)
    ...
    def _collect_cves_from_installed_packages(self):
        ...
        for src_pkg_name in self.installed_packages:
            ...
            cve_product = self._get_cve_products_from_source_package_name(src_pkg_name)
            pi = self.installed_packages[src_pkg_name]
            self._check_cves(conn, pi, cve_product)

        conn.close()

    def _check_cves(self, conn: sqlite3.Connection, pkginfo: PackageInfo, cve_product: CveProduct) -> None:
        ...
        for cp in cve_product:
            ...
            pv = pkginfo[0].upstream_version
            src_pkg_name = pkginfo[0].src_pkg_name

            cve_cursor =conn.execute("SELECT DISTINCT ID FROM PRODUCTS WHERE PRODUCT IS ? AND VENDOR LIKE ?", (product, vendor))
            for cverow in cve_cursor:
                cveid = cverow[0]
                vulnerable = False
                product_cursor = conn.execute("SELECT VERSION_START, OPERATOR_START, VERSION_END, OPERATOR_START  FROM PRODUCTS WHERE ID IS ? AND PRODUCT IS ? AND VENDOR LIKE ?", (cveid, product, vendor))
                for row in product_cursor:
                    (version_start, operator_start, version_end, operator_end) = row

                    vuln_status = CveStatus.CVE_STATUS_PATCHED
                    vulnerable = lib.check_affected(pv, version_start, operator_start, version_end, operator_end)
                    if vulnerable:
    ...
    def _parse_node_and_insert(self, conn, node, cveId):
        ...
        conn.executemany("insert into PRODUCTS values (?, ?, ?, ?, ?, ?, ?)", _cpe_generator()).close()
    ...
    def _update_db(self, conn, elt):
        ...
        conn.execute("insert or replace into NVD values (?, ?, ?, ?, ?, ?, ?, ?)",
                    [cveId.strip(), vulnStatus.strip(), cveDesc.strip(), cvssv2, cvssv3, date.strip(), accessVector.strip(), vectorString.strip()]).close()
    ...
    def _nvd_request_next(self, url: str, request_args: Any) -> str:
        ...
        request = urllib.request.Request(url + "?" + urllib.parse.urlencode(request_args))
        if self.nvd_api_key:
